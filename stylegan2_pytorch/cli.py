...
def run_training(rank, world_size, model_args, data, load_from, new, num_train_steps, name, seed):
    ...
    retry_call(model.evaluate, tries=3, exceptions=NanException)
...
def train_from_folder(
    ...
    if generate is not None:
        model = Trainer(**model_args)
        model.load(load_from)
        save_dir = model.generate_(num=num_generate, type=generate)
        print(f'sample images generated at {save_dir}')
        return
