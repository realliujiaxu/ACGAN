...
from stylegan2_pytorch.augment import AugWrapper
...
class StyleGAN2(nn.Module):
    def __init__(self, image_size, n_attr=40, latent_dim=512, fmap_max=512, style_depth=8, network_capacity=16,
                 transparent=False, fp16=False, cl_reg=False,
                 steps=1, lr=1e-4, ttur_mult=2, fq_layers=[], fq_dict_size=256, attn_layers=[], no_const=False,
                 lr_mlp=0.1, rank=0):
        super().__init__()
        print(
            f'Constructing StyleGAN2 with latent dim {latent_dim}, fmap {fmap_max}, network_capacity {network_capacity}')
        self.lr = lr
        self.steps = steps
        self.ema_updater = EMA(0.995)

        self.S = StyleVectorizer(latent_dim, style_depth, lr_mul=lr_mlp)
        self.G = Generator(image_size, latent_dim + n_attr, network_capacity, transparent=transparent,
        ...
        self.GE = Generator(image_size, latent_dim + n_attr, network_capacity, transparent=transparent,
                            attn_layers=attn_layers, no_const=no_const, fmap_max=fmap_max)
